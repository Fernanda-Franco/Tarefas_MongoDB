...
app.get("/tarefas", async (req, res) => {
    //Realiza a busca de todos os documentos na coleção
...
app.get("/tarefas/:id", async (req, res) => {
    ...
    console.log(err);
    res.json(500).json({ message: "Um erro aconteceu" });
    } 
});
...
//Retorna o objeto encontrado
    ...
    if(tarefaExistente) {
        re.json({ message: "Tarefa editada"});
    } else {
        res.status(404).json({ message: "Tarefa não encontrada" });
    }
    } catch(err) {
...
app.delete("/tarefas/:id", async (req, res) => {
    try {
        ...
        if(tarefaExistente) {
            res.json({ message: "Tarefa excluida", tarefasRestantes });
        } else {
            res.status(404).json({ message: "Tarefa não encontrada." });
        }
    } catch (err) {
